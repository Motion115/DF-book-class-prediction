...
#读取数据集
with open("my_train.csv", encoding='utf-8') as csvfile:
    csv_reader = csv.reader(csvfile)
    for row in csv_reader:
        if len(row) >= 2:  # 确保每行至少有两个单元格
            title = row[0]
            y = row[1]
...
# 5:1 划分训练集、验证集
input_ids_train, input_ids_valid = input_ids[idxes[:50000]], input_ids[idxes[50000:]]
input_masks_train, input_masks_valid = input_masks[idxes[:50000]], input_masks[idxes[50000:]] 
input_types_train, input_types_valid = input_types[idxes[:50000]], input_types[idxes[50000:]]

y_train, y_valid= labels[idxes[:50000]], labels[idxes[50000:]]
